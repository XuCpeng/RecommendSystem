...
object ALSTrainer {
  ...
  def adjustALSParams(trainData:RDD[Rating]): Unit ={
    val result = for(rank <- Array(30,40,50,60,70); lambda <- Array(1, 0.1, 0.001)) //尝试的参数
      yield {
  ...
  def getRmse(model:MatrixFactorizationModel, trainData:RDD[Rating]):Double={
    ...
    val predictRating = model.predict(userMovies)  //预测

    val real = trainData.map(item => ((item.user,item.product),item.rating)) //真实值(userd对product的打分为rating)
    val predict = predictRating.map(item => ((item.user,item.product),item.rating)) //预测值(userd对product的打分为rating)

    sqrt(
      real.join(predict).map{case ((_, _),(real,pre))=>
        // 真实值和预测值之间的两个差值
        val err = real - pre
        err * err
      }.mean() //mean求平均值
